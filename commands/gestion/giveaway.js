      ...
      if (args[0] === "reroll") {
        if (!args.lenght > 0) {
          ...
          message.channel.messages.fetch(db.get(`last${message.guild.id}`)).then(m => {
            ...
            message.channel.send(`Félicitations à ${winner} qui gagne ${db.get(`gain${message.guild.id}<a:Giveaway:980077234138259497>`)}`)
          })
        } else {
          message.channel.messages.fetch(args[0]).then(m => {
            ...
            if (!winner) return message.channel.send(`Aucun participant valide`)
            message.channel.send(`Félicitations à ${winner} qui gagne ${db.get(`gain${message.guild.id}`)}`)
